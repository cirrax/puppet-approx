...
describe 'approx' do
  ...
  shared_examples 'approx shared examples' do
    ...
    it {
      is_expected.to contain_package('approx')
        .with_ensure('installed')
    }
    it {
      is_expected.to contain_concat(params[:conffile])
        .with_owner('root')
        .with_group('root')
        .with_mode('0644')
    ...
    it {
      is_expected.to contain_concat__fragment('approx.conf_header')
        .with_target(params[:conffile])
        .with_order('00')
    }

    it {
      is_expected.to contain_file(params[:cache])
  ...
  context 'with ensure_cache false' do
    ...
    it {
      is_expected.not_to contain_file(params[:cache])
    }
  end
  ...
  context 'with creation of repositories' do
    ...
    it_behaves_like 'approx shared examples'
    it {
      is_expected.to contain_approx__repository('debian')
